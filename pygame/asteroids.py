...
def handle_input():
    ...
    global bullet_x, bullet_y, bullet_angle, no_bullest
...
def isCollosion(enemyX, enemyY, bulletX, bulletY, dist):
...
def game_logic():
    ...
    for i in range(0,no_asteroids):
        ...
        if isCollosion(ship_x,ship_y,asteroid_x[i], asteroid_y[i], 27):
            print('Game Over')
            exit()
            
    for i in range(0, no_bullest): 
        for j in range(0,no_asteroids):
            if isCollosion(bullet_x[i], bullet_y[i], asteroid_x[j], asteroid_y[j], 50):
                score +=1
                asteroid_x[j] = ( random.randint(0,WIDTH))
                asteroid_x[j] = ( random. randint(0,HEIGHT))
